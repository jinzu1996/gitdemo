...
import ACCOUNT_SOURCE from '@salesforce/schema/Store_Account__c.Location__c';
...
export default class Executivevi extends NavigationMixin(LightningElement)
{
    ...
    // accountInfo;
    @wire(getPicklistValues,{recordTypeId : '',fieldApiName : ACCOUNT_SOURCE})//$accountInfo.data.defaultRecordTypeId
    ...
    @wire(getPicklistValuesByRecordType,{recordTypeId : '',objectApiName : ACCOUNT_OBJECT})
        wiredRecordtypeValues({data, error})
        {
            if(data){
              //  this.pickListvaluesByRecordType = data.picklistFieldValues.Executive_Name__c.values;
                this.accountsource = data.picklistFieldValues.Executive_Name__c.values;
            }
            if(error){
                console.log(error);
            }
  ...
  @track orglis;
                        ...
                        if(row != '' && row != null && row != 'null' && row != 'undefined' && row != undefined)
                        {
                            ...
                            if(row.BeatPlans__r != '' && row.BeatPlans__r != null && row.BeatPlans__r != 'null' && row.BeatPlans__r != 'undefined' && row.BeatPlans__r != undefined)
                            {
                                ...
                                row.BeatPlans__r.forEach((row1) => 
                                {
                                    ...
                                    var chk = row1.Store_Status__c;//Status__c;
                                    var re = chk.replace('<br>', ' ');
    ...
    wiredSobjects(result) 
    {
        ...
        if(result.data) 
        {
            ...
            console.log('Populate results result --- ', test);
    ...
    {
        console.log('Update contact handler--',event);
